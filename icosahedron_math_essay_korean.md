...
$$36° + \text{밑각} \times 2 = 180° \;\;\Longrightarrow\;\; \text{밑각} = \frac{144°}{2} = 72°$$
...
여기서 짚고 넘어갈 점이 있다. 곧 정이십면체의 외접구 반지름 계수로 다시 등장할 0.951이라는 수는, 반올림되거나 측정된 근삿값이 아니다. √5 하나로 완전히 표현되는 **정확한 대수적 값**이다. "0.951…"의 모든 자릿수가 이 근호식으로 완벽히 결정되며, 어떤 수치적 근사도 개입하지 않는다. 이 사실은 8절에서 이 수가 다시 등장할 때도 그대로 유지된다.
...
0.951과 0.809는 모순이 아니라 서로 다른 두 질문(꼭짓점까지인가, 모서리 중점까지인가)에 대한 서로 다른 두 답이다. 어떤 계산에서든 자신이 쓰는 R이 이 중 무엇인지 먼저 확정해야 한다. 그리고 6절에서 확인했듯, 특히 0.951이라는 수는 측정되거나 어림된 값이 아니라 — 정이십면체와 무관하게 유도된 정확한 대수적 수 sin 72°에 ℓ을 곱한 것이, 바로 이 도형의 두 핵심 반지름 중 하나로 다시 나타난 것이다.
...
정이십면체는 조합적으로 20-30-12(면-모서리-꼭짓점)이고, 엣지 ℓ에 대해 0.951ℓ(외접구, sin 72°)과 0.809ℓ(중접구, sin 54° = φ/2)이라는 두 특성 길이를 갖는다. 이 값들은 전부 하나의 계산에서 유도된다 — 36°-72°-72° 황금 삼각형의 밑각을 이등분하고, 닮음비로 b² + b − 1 = 0 을 세우고, 근의 공식으로 b = (√5−1)/2 = 1/φ 를 얻는 것. 여기서 cos 72°가 나오고, 피타고라스 항등식으로 sin 72°가 나오고, 그 값들이 다면체의 반지름이 된다.

정이십면체의 진정으로 놀라운 점은 단지 황금비를 품고 있다는 사실 자체가 아니다. 오히려 **동일한 하나의 이차방정식**이 황금비를, 정오각형의 특징적인 삼각비들을, 그리고 결국 이 다면체의 핵심 반지름들까지 전부 만들어낸다는 데에 있다. 언뜻 서로 무관해 보이는 상수들의 나열 — 0.618, 0.809, 0.951, 1.618 — 은 사실 하나의 수학적 이야기이며, 단 한 번 쓰인 그 이야기가 도형의 모든 규모에서 메아리치고 있는 것이다. 유도에 쓰인 도구는 내각의 합, 닮음, 근의 공식, 삼각비 — 전부 고등학교 수학이며, 그 출발점에는 근의 공식 한 줄이 있다.
...
이 수치 검산은, 정이십면체의 좌표 정의만을 이용해 8절의 닫힌 형태 결과를 독립적으로 재확인해 준다.
